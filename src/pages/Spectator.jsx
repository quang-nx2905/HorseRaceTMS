...
function Spectator() {
    ...
    useEffect(() => {
        const fetchStreams = async () => {
            try {
                ...
                const mappedLives = lives.map(mapRace).sort((a, b) => (b.youtubeId ? 1 : 0) - (a.youtubeId ? 1 : 0));
                const mappedUpcomings = upcomings.map(mapRace).sort((a, b) => (b.youtubeId ? 1 : 0) - (a.youtubeId ? 1 : 0));

                setLiveRaces(mappedLives);
                setUpcomingRaces(mappedUpcomings);
                
                const targetRace = location.state?.targetRace;
                const targetRaceId = targetRace?.raceId;
                let targetStream = null;
                
                if (targetRaceId) {
                    const found = data.find(r => r.raceId === targetRaceId);
                    if (found) {
                        targetStream = mapRace(found);
                    } else if (targetRace) {
                        // Race not in active streams (no link or wrong status), use it directly so black screen shows
                        targetStream = {
                            ...targetRace,
                            race: targetRace.raceName || targetRace.race,
                            tournamentName: targetRace.tournamentName || "Uncategorized",
                            youtubeId: targetRace.youtubeId || null
                        };
                    }
                }
                ...
                } else if (mappedLives.length > 0) setSelectedStream(mappedLives[0]);
                else if (mappedUpcomings.length > 0) setSelectedStream(mappedUpcomings[0]);
    ...
    return (
        <div className="pb-12">
                    ...
                    <div className="bg-white border border-zinc-200 rounded-3xl overflow-hidden shadow-sm">
                        ...
                        <div className="p-5 flex flex-col md:flex-row md:items-center justify-between border-b border-zinc-100 gap-4">
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                <span className="px-3 py-1.5 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl text-xs font-bold flex items-center">
                                    <Trophy className="w-3 h-3 inline mr-1" />
                                    {selectedStream.prize}
                                </span>
                                <button 
                                    onClick={() => setTheaterMode(true)}
                                    className="px-3 py-1.5 bg-zinc-900 border border-zinc-700 text-white hover:bg-black rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors shadow-sm ml-2"
                                >
                                            ...
                                            {races.map((r) => (
                                                <button
                                                    ...
                                                    className={`text-left p-4 rounded-2xl border transition-all shadow-sm ${
                                                        selectedStream?.raceId === r.raceId
                                                            ? "bg-gradient-to-br from-zinc-900 to-zinc-950 border-zinc-800 text-white shadow-lg ring-2 ring-red-500/20"
                                                            : r.youtubeId 
                                                                ? "bg-red-50/40 border-red-100 hover:border-red-300 hover:shadow-md text-zinc-800"
                                                                : "bg-white border-zinc-100 hover:border-zinc-300 hover:shadow-md text-zinc-700"
                                                    }`}
                                                >
                                                    <div className="flex items-center justify-between mb-2">
                                                        <p className={`font-bold text-sm truncate pr-2 ${selectedStream?.raceId === r.raceId ? "text-white" : "text-zinc-900"}`}>{r.race}</p>
                                                        {r.youtubeId ? (
                                                            ...
                                                            <span className={`flex items-center gap-1 text-[10px] font-black flex-shrink-0 ${selectedStream?.raceId === r.raceId ? "text-zinc-500" : "text-zinc-400"}`}>
                                                                <span className={`w-1.5 h-1.5 rounded-full ${selectedStream?.raceId === r.raceId ? "bg-zinc-500" : "bg-zinc-400"}`} />
                                                                ONGOING
                                                            </span>
                                                        )}
                                                    </div>
                                                    <p className={`text-xs mb-2 truncate ${selectedStream?.raceId === r.raceId ? "text-zinc-400" : "text-zinc-500"}`}><MapPin className="w-3 h-3 inline mr-1" />{r.track}</p>
                                                    <div className="flex items-center justify-end text-xs font-bold mt-3 pt-3 border-t border-white/5">
                                                        <span className={`flex items-center gap-1 ${selectedStream?.raceId === r.raceId ? "text-amber-400" : "text-amber-600"}`}>
            ...
            {theaterMode && (
                <div className="fixed inset-0 z-[100] bg-black flex flex-col p-4 md:p-8 animate-in fade-in duration-200">
                    <div className="flex justify-end mb-4">
                        <button 
                            onClick={() => setTheaterMode(false)} 
                            className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl transition font-bold flex items-center gap-2 border border-white/10"
                        >
                            <X className="w-4 h-4" /> Exit Theater Mode
                        </button>
                    </div>
                    <div className="flex-1 w-full flex items-stretch justify-center gap-4 lg:gap-6 pb-4 md:pb-8 px-2 md:px-6 h-full">
                        <div className="flex-1 w-full max-w-6xl aspect-video bg-zinc-950 rounded-[2rem] overflow-hidden shadow-2xl ring-1 ring-white/10 relative">
                            {selectedStream.youtubeId ? (
                                <iframe
                                ...
                                <div className="flex h-full flex-col items-center justify-center text-center">
                                    <div className="mb-5 flex h-20 w-20 items-center justify-center rounded-3xl border border-white/10 bg-white/5"><Play className="h-8 w-8 text-zinc-500" /></div>
                                    <h2 className="text-2xl font-black text-white">No YouTube Link Attached</h2>
                                    <p className="mt-2 text-zinc-400">Please provide a valid YouTube ID to watch in theater mode.</p>
                                </div>
                            )}
                        </div>

                        {/* Transparent Chat Overlay */}
                        <div className="w-[350px] lg:w-[420px] flex-shrink-0 bg-black/40 backdrop-blur-md border border-white/10 rounded-3xl flex flex-col z-20 transition-all pointer-events-auto overflow-hidden shadow-2xl">
                                <div className="p-4 border-b border-white/10 flex-shrink-0 flex items-center justify-between bg-black/20">
                                    <h3 className="font-bold text-white text-sm flex items-center gap-2">
                                        <MessageSquare className={`w-4 h-4 ${chatMode === "youtube" ? "text-red-500" : "text-violet-400"}`} />
                                        Live Chat
                                    </h3>
                                    <div className="flex bg-white/10 p-1 rounded-lg">
                                        <button 
                                            onClick={() => setChatMode("youtube")}
                                            className={`px-2 py-1 text-[10px] font-bold rounded-md transition-colors ${chatMode === "youtube" ? "bg-white text-red-600 shadow-sm" : "text-white/60 hover:text-white"}`}
                                        >
                                            YouTube
                                        </button>
                                        <button 
                                            onClick={() => setChatMode("app")}
                                            className={`px-2 py-1 text-[10px] font-bold rounded-md transition-colors ${chatMode === "app" ? "bg-white text-violet-600 shadow-sm" : "text-white/60 hover:text-white"}`}
                                        >
                                            App
                                        </button>
                                    </div>
                                ...
                                {chatMode === "youtube" ? (
                                    <div className="flex-1 relative bg-black/50 overflow-hidden mix-blend-screen opacity-90">
                                        {selectedStream.youtubeId ? (
                                            <iframe
                                                src={`https://www.youtube.com/live_chat?v=${selectedStream.youtubeId}&embed_domain=${window.location.hostname}&dark_theme=1`}
                                                className="absolute inset-0 w-full h-full border-0"
                                                title="YouTube live chat"
                                            ></iframe>
                                        ) : (
                                            <div className="flex flex-col items-center justify-center h-full text-white/40 p-6 text-center">
                                                ...
                                                <p className="text-xs font-medium">Add a YouTube link.</p>
                                            </div>
                                        )}
                                    </div>
                                ) : (
                                    <>
                                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                            {reactions.length === 0 && (
                                                <div className="text-center text-white/50 text-xs mt-10">No comments yet.</div>
                                            )}
                                            {reactions.map((r, i) => (
                                                <div key={i} className="flex flex-col">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <span className={`text-[10px] font-bold ${r.user === "You" ? "text-amber-400" : "text-white/70"}`}>{r.user}</span>
                                                        <span className="text-[10px] text-white/40">{r.time}</span>
                                                    </div>
                                                    <div className="text-sm text-white/90 leading-relaxed drop-shadow-md">
                                                        {r.text}
                                                    </div>
                                                </div>
                                            ))}
                                        ...
                                        <div className="p-3 border-t border-white/10 flex-shrink-0 bg-black/20">
                                            <div className="flex gap-2">
                                                <input
                                                    type="text"
                                                    placeholder="Chat..."
                                                    ...
                                                    onChange={(e) => setChatMsg(e.target.value)}
                                                    onKeyDown={(e) => e.key === "Enter" && handleSend()}
                                                    className="flex-1 px-3 py-2 bg-white/10 border border-white/10 focus:border-amber-400/50 rounded-xl outline-none text-xs text-white placeholder-white/40 backdrop-blur-md"
                                                />
                                                <button
                                                    onClick={handleSend}
                                                    className="w-9 h-9 flex-shrink-0 rounded-xl bg-amber-500 hover:bg-amber-600 text-zinc-950 flex items-center justify-center transition-all"
                                                >
